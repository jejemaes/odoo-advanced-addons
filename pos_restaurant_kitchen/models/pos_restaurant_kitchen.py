...
class RestaurantKitchen(models.Model):
    ...
    def action_mark_all_as_print(self):
        orders = self.env['pos.order'].search([('kitchen_ids', 'in', self.ids), ('print_count', '=', 0)])
        orders.write({'print_count': 1})
        return True

    def action_order_dashbaord(self):
        action = self.env.ref('pos_restaurant_kitchen.pos_order_action').read()[0]
    ...
    def get_order_line(self, order):
        ...
        return self.env['pos.order.line']  # TODO
